...
public class FileUploadController {
	 ...
	 * public String addItemPost(@Valid @ModelAttribute("item") UserModel
	 * user,BindingResult result, HttpServletRequest request,Model model) { /*
	 * MultipartFile itemImage = item.getItemImage(); String rootDirectory =
	 * request.getSession().getServletContext().getRealPath("/"); path =
	 * Paths.get(rootDirectory +
	 * "\\WEB-INF\\resources\\images\\"+item.getItemId()+".png");
	 * 
	 * 
	 * if (itemImage != null && !itemImage.isEmpty()) { try {
	 * itemImage.transferTo(new File(path.toString())); } catch (Exception e) {
	 * e.printStackTrace(); throw new RuntimeException(
	 * "item image saving failed.", e); } }
	 * 
	 ...
	 */
	/*
	 * @RequestMapping("/upload") public ModelAndView
	 * myprof(@ModelAttribute("product") UserModel user)
	 * 
	 * {
	 * 
	 ...
	 * System.out.println("ddddddddddddddd-----------"); String name =
	 * file.getOriginalFilename(); String path = "E:\\Test\\uploads"; if
	 * (!file.isEmpty()) { try { System.out.println("ddddddddddddddd"); File
	 * imgDirectory = new File(path); if(!imgDirectory.exists())
	 * {System.out.println("ddddddddddddddd"); imgDirectory.mkdir(); }
	 * 
	 * byte[] bytes = file.getBytes(); BufferedOutputStream stream = new
	 * BufferedOutputStream(new FileOutputStream(new File(path+"\\"+name)));
	 * stream.write(bytes); stream.close();
	 * System.out.println("ddddddddddddddd"); return
